function (x, w = rep(1/nrow(x), nrow(x)), y = initialy(x), verbose = -1, 
          alpha = 5, c = 1, sigmatol = 10^-8, integraltol = 10^-4, 
          ytol = 10^-4, Jtol = 0.001, chtol = 10^-6) 
{
  ...
  if (length(w) != nrow(x)) 
    stop("there must be one w_i for each observation")
  if (sum(w <= 0)) 
  ...
  opts[3] <- sigmatol
  opts[4] <- 15000
  opts[5] <- as.double(verbose)
  opts[6] <- integraltol
  opts[7] <- as.double(alpha)
  opts[8] <- 1 * 10^(-11)
  chopts <- paste("Qt", sep = "")
