...
SimModel1 <- function(M, n, p0, mu1, sigma1)
  # [Univariate] Null: Normal, Nonnull: Normal
{
  library(locfdr)
  ...
  for ( r in 1:M ) {
    ...
    z <- c(z0, z1)
    tmp <- p0*dnorm(z)
    f <- tmp + (1-p0)*dnorm(z, mean = mu1, sd = sigma1)
    localFDR <- tmp/f
...
Res.1 <- SimModel1(M = 500, p0 = 0.95, n = 1000, mu1 = 3, sigma1 = .5)
Res.2 <- SimModel1(M = 500, p0 = 0.90, n = 1000, mu1 = 3, sigma1 = .5)
Res.3 <- SimModel1(M = 500, p0 = 0.80, n = 1000, mu1 = 3, sigma1 = .5)
save.image(file = "SimUniNormal.RData")

load(file = "SimUniNormal.RData")

png(file = "UniNormalp95.png", height = 600, width = 900)
Draw.boxplots(Res.1)
dev.off()

png(file = "UniNormalp90.png", height = 600, width = 900)
Draw.boxplots(Res.2)
dev.off()

png(file = "UniNormalp80.png", height = 600, width = 900)
Draw.boxplots(Res.3)
dev.off()
